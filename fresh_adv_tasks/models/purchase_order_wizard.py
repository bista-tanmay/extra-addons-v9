    ...
    def confirm_purchase_orders(self):
        ...
        for each_current_active_ids in all_active_ids:
            ...
            for each_copy in copy_curr_active_ids:
                ...
                if browse_copy_active_ids.id == browse_active_ids.id or browse_active_ids.state == 'cancel' or browse_copy_active_ids.state == 'cancel':
                    continue
                if browse_active_ids.partner_id.id == browse_copy_active_ids.partner_id.id and \
                   browse_active_ids.payment_term_id.id == browse_copy_active_ids.payment_term_id.id:
...
class StockPickingWizard(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        not_transferred_name_list = ''
        not_transferred_state_list = ''
        all_active_ids = self._context.get('active_ids')
        for each_current_active_ids in all_active_ids:
            browse_active_ids = self.env['stock.picking'].browse(each_current_active_ids)
            if browse_active_ids.state != 'assigned':
                if not_transferred_name_list == '' and not_transferred_state_list == '':
                    not_transferred_name_list += browse_active_ids.name
                    not_transferred_state_list += browse_active_ids.state
                else:
                    not_transferred_name_list += ','+browse_active_ids.name
                    not_transferred_state_list += ','+browse_active_ids.state
        if not_transferred_name_list != '':
            raise UserError(_("Stock Picking %s are in %s state(s) respectively") % (not_transferred_name_list, not_transferred_state_list))
        return res
